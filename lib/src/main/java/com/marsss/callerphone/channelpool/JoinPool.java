...
public class JoinPool implements Command {
    ...
    public void runCommand(GuildMessageReceivedEvent e) {
        ...
        if (stat == 413) {
            ...
            MESSAGE.reply("This channel is already in a pool.").queue(m -> {
                m.editMessage(m.getContentRaw() + "\nLeave pool with: `" + Callerphone.Prefix + "leavepool`").queue();
            });
        } else if (stat == 404) {
            MESSAGE.reply("Requested pool ID *(" + host + ")* does not exist.").queue();
        } else if (stat == 200) {
